...
function PopupWithForm(props) {
  return (
    <div className={`popup popup_${props.name} ${props.isOpen && 'popup_opened'}`} id={`popup_${props.name}`}>
      <form className="popup__form popup__form_edit" name="popupFormEdit" id="popupFormEdit" noValidate>
        <h2 className="popup__title">{`${props.title}`}</h2>
        {props.children}
        <button type="submit" className="popup__btn-submit button">Сохранить</button>
